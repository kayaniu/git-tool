...
var fs = require('fs')
    ...
    ,_ = require('lodash')
    ,fsUtils = require('./util/filesystem')
    ,mkdirp = require('mkdirp')
    ,sc = require('./stash-clone/stash-clone')
    ,git = require('./git-utils/git-utils')
    ,parseArgs = require('minimist');

var ops = {checkout: 'checkout', pull:'pull'};
...
function main(workingDir, argv){
    ...
    if (command.toLowerCase() == ops.pull){
        pullCommand(workingDir, argv);
    }
    else if (command.toLowerCase() == ops.checkout){
        checkoutCommand(workingDir, argv);
    }
    else {
        console.log('Invalid arguments');
        showHelp();
...
/**
 * Given a working directory, find all git repositories and execute a pull command on each
 * @param wd
 * @param argv
 */
function pullCommand(wd, argv){

    var options = null;
    var select = argv.select;

    var handler = function handleFolder(folders){
        pull(folders, options);
    }

    getDirectories(wd, git.isGitRepository, function(err, folders){
...
/**
 * Given a working directory, find all git repositories and execute a checkout command on each to a specified branch
 * @param wd
 * @param argv - must contain a property 'b' with value equal to the branch name
 */
function checkoutCommand(wd, argv){
    ...
    var options = {branch: argv.b};
    var select = argv.select;

    var handler = function handleFolder(folders){
        checkout(folders, options);
    }
...
function handleSelection(selection, action, options){
    ...
    selection.forEach(function(o){
        ...
        action(o.path, options, function(err, command, stdout){
            ...
            console.log("Running " + command + " on " + o.name);
            console.log(stdout);
        });
    });
